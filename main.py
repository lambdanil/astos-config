...
def main(args):
    ...
    timezone = str(f"/usr/share/zoneinfo/Europe/Prague")
...
#    os.system("cp --reflink=auto -r /mnt/var/* /mnt/.snapshots/var/var-tmp")
    ...
    if DesktopInstall == 1:
        ...
        excode = int(os.system("pacstrap /mnt flatpak gnome gnome-extra gnome-themes-extra gdm pipewire pipewire-pulse sudo bluez-utils gdb htop noto-fonts-cjk podman python-pip sbcl ttf-fira-code vim wget xcb-util-image xcb-util-keysyms xcb-util-wm xcb-util-renderutil zram-generator"))
        if excode != 0:
            print("Failed to download packages!")
            sys.exit()
        username = "jan"
...
#        os.system("cp --reflink=auto -r /mnt/var/* /mnt/.snapshots/var/var-tmp")
        ...
        os.system("tar -xvf ./etc.tar.xz")
        os.system("arch-chroot /mnt touch /etc/subuid /etc/subguid")
        os.system(f"usermod --add-subuids 100000-165535 --add-subgids 100000-165535 {username}")
        os.system("cp ./etc/* /mnt/etc -r")
        os.system("arch-chroot /mnt locale-gen")
        os.system("arch-chroot /mnt systemctl enable zram")
        os.system("arch-chroot /mnt systemctl enable bluetooth")
